...
int				rl_insert_ch(int c)
{
	...
	if (line[g_rl.cc])
	{
		rl_putstr_wrap(line, g_rl.cc);
		rl_cur_fromto(ft_strlenz(line), g_rl.cc);
	}
	return (0);
}
...
int				rl_ctrl_del_ch(int c)
{
	...
	rl_putnchar_wrap(' ', ft_strlenz(line + g_rl.cc - 1), g_rl.cc - 1);
	rl_cur_fromto(ft_strlenz(line), g_rl.cc - 1);
