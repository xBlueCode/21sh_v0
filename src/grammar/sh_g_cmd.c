...
//	t_and_or	*and_or;
	...
	if (!ast || ast->op != SH_GR_COMPLETE_CMD)
		return (0);
	com_cmd = sh_g_com_cmd_new();
	list = ast->left;
	while (list)
	{
	//	if (sh_g_and_or(list->left, &and_or))
	//		ft_lst_addlast(&com_cmd.lst_and_or, ft_lstnew(&and_or, sizeof(t_and_or*)));
		if (list->data)
			ft_dstrins_ch(com_cmd->sep, -1, ((t_token*)(list->data))->t);
		list = list->right;
	}
	return (com_cmd);
}
...
//	void	*cmd_core;
	...
	if (!ast || ast->op != SH_GR_CMD)
		return (0);
	cmd.lst_redirect = NULL;
	redir_list = ast->right;
	ast_core = ast->left;
	while (redir_list)
	{
		if (sh_g_redir(redir_list->left, &redir))
			ft_lst_addlast(&cmd.lst_redirect, ft_lstnew(&redir, sizeof(t_and_or*)));
